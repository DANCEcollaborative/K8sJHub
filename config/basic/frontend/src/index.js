...
const plugin = {
    ...
    activate: (app, restorer) => {
        const wsURL = window.CHAT_WS_URL || process.env.CHAT_WS_URL;
    ...
    `;
        ...
        chatWidget.node.querySelector('#joinBtn')?.addEventListener('click', () => {
            const room = chatWidget.node.querySelector('#roomInput').value;
            if (room) {
                if (currentRoom) {
        ...
        chatWidget.node.querySelector('#sendBtn')?.addEventListener('click', () => {
            const input = chatWidget.node.querySelector('#chatInput');
